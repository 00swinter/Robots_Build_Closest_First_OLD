...
local config = {
  update_interval = 15,
  reserve_bots    = 2,    -- keep this many idle
  accel_gain      = 1, -- how strongly radius reacts to idle bots
  damping         = 0.2, -- velocity factor (0..1)
  max_speed       = 2.0,  -- clamp velocity (tiles/tick)
  min_radius      = 3
}
...
local function newTick()
  ...
  storage.avg_rb = storage.avg_rb or { buffer = {}, max = 5, index = 0, sum=0}
  ...
  local err = (moving_average_avail - config.reserve_bots)
